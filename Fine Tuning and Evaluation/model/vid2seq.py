...
class Vid2Seq(torch.nn.Module):
    ...
    # added audio parameter fopr forward passes.
    def forward(self, video, input_tokenized, output_tokenized, audio=None):
        ...
        outputs = self.t5_model(
            encoder_outputs=BaseModelOutput(last_hidden_state=final_hidden_state),  # <-- Changed: use fused multimodal embeddings
    ...
    def generate(
        ...
        if self.use_audio and audio is not None:
            # append audio hidden states andattention masks
            hidden_states.append(audio) 
            attention_masks.append(atts_audio)  
        ...
        outputs = self.t5_model.generate(
            encoder_outputs=BaseModelOutput(last_hidden_state=final_hidden_state), # Fused multimodel embeddings.
            attention_mask=encoder_atts,
