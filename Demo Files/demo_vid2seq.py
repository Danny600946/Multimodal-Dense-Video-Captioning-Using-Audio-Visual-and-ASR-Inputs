# File: demo_vid2seq.py
# Original Author(s): VidChapters Team (https://github.com/antoyang/VidChapters)
# Modified by: Daniel Vousden
#
# Description:
# This script is adapted from the original VidChapters inference pipeline. It performs multimodal inference
# using the Vid2Seq model by incorporating audio features alongside video and ASR inputs.
#
# Key Modifications:
# - Added loading and integration of pre-extracted Wav2Vec audio features (.pt format).
# - Reserved token space for audio in the input sequence (adjusted max input token limit).
# - Forward pass updated to include audio input during caption generation.
#
# License: MIT (original project license preserved)
...
print(f"[DEBUG] Total ASR segments loaded: {len(texts)}")
if texts:
    print(f"[DEBUG] First ASR segment: '{texts[0]}'")
else:
    print("[DEBUG] No ASR segments found!")
...
if not sub['text']:
    ...
    # Changed token limit to reserve space for audio
...
# Load pre-extracted audio features, shape: [1, 100, 768]
print("loading audio features")
audio_features = torch.load("audio_features.pt").to(device) 
audio_features = audio_features.unsqueeze(0) if audio_features.dim() == 2 else audio_features

# Error checking shape
print(f"[INFO] Audio features loaded: {audio_features.shape}")
assert audio_features.shape[1] == 100, f"Expected 100 audio tokens, got {audio_features.shape[1]}"
...
print(f"[DEBUG] Raw model output string: {output[0]}")
